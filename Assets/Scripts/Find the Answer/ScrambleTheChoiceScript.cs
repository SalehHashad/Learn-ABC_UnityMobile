...
/// </summary>
public class ScrambleTheChoiceScript : GameParent
{
    ...
    void Start()
    {
        if (isArabic)
        {
            alphabetText.text = ArabicFixerTool.FixLine(arabicAlphabet[alphabetIndex].ToString());
        }
        else
        {
    ...
    protected override void InitAlphabets()
    {
        ...
        for (int i = 0; i < 3; i++)
            transform.GetChild(i).GetComponent<LetterTileBehaviour>().setObj(GetComponent<ScrambleTheChoiceScript>(), problemTile[i]);
    }
    ...
    public void AnimationTrigger(ObjectWord obj) {
        objectName.text = ArabicFixerTool.FixLine( obj.objectName);
        alphabetText.text = ArabicFixerTool.FixLine(obj.alphabet);
